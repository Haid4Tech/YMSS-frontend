...
export default function ExamDetailPage() {
  ...
  const examStats = {
    ...
    highestScore: grades.length > 0 
      ? Math.max(...grades.map(g => g.marks || 0))
      : "N/A",
    lowestScore: grades.length > 0 
      ? Math.min(...grades.map(g => g.marks || 0))
      : "N/A",
    passRate: grades.length > 0 
      ? ((grades.filter(g => (g.marks || 0) >= (exam?.totalMarks || 100) * 0.6).length / grades.length) * 100).toFixed(1)
      : "N/A",
    median: grades.length > 0 
      ? (() => {
          const sorted = grades.map(g => g.marks || 0).sort((a, b) => a - b);
  ...
  const scoreRanges = [
    { range: `${Math.floor(totalMarks * 0.9)}-${totalMarks}`, count: grades.filter(g => (g.marks || 0) >= totalMarks * 0.9).length, color: "#10B981" },
    { range: `${Math.floor(totalMarks * 0.8)}-${Math.floor(totalMarks * 0.9) - 1}`, count: grades.filter(g => (g.marks || 0) >= totalMarks * 0.8 && (g.marks || 0) < totalMarks * 0.9).length, color: "#3B82F6" },
    { range: `${Math.floor(totalMarks * 0.7)}-${Math.floor(totalMarks * 0.8) - 1}`, count: grades.filter(g => (g.marks || 0) >= totalMarks * 0.7 && (g.marks || 0) < totalMarks * 0.8).length, color: "#F59E0B" },
    { range: `${Math.floor(totalMarks * 0.6)}-${Math.floor(totalMarks * 0.7) - 1}`, count: grades.filter(g => (g.marks || 0) >= totalMarks * 0.6 && (g.marks || 0) < totalMarks * 0.7).length, color: "#EF4444" },
    { range: `0-${Math.floor(totalMarks * 0.6) - 1}`, count: grades.filter(g => (g.marks || 0) < totalMarks * 0.6).length, color: "#6B7280" }
  ];
  ...
  const rankedResults = grades
    .map(grade => {
      ...
      return {
        grade,
        student,
        percentage: exam?.totalMarks ? ((grade.marks || 0) / exam.totalMarks * 100).toFixed(1) : "0"
      };
    })
  ...
  const standardDeviation = grades.length > 0 
    ? (() => {
        const mean = parseFloat(examStats.averageScore);
        const variance = grades.reduce((sum, grade) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold text-primary">{examStats.totalStudents}</div>
            <p className="text-sm text-muted-foreground">Students</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold text-blue-600">{examStats.averageScore}</div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold text-red-600">{examStats.lowestScore}</div>
            <p className="text-sm text-muted-foreground">Lowest</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold text-purple-600">{examStats.passRate}%</div>
            <p className="text-sm text-muted-foreground">Pass Rate</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold text-orange-600">{examStats.median}</div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">{exam.subject?.name || "Not specified"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm">{exam.duration ? `${exam.duration} minutes` : "Not set"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm">{exam.instructions || "No instructions provided"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-3xl font-bold text-green-600">
                  {grades.filter(g => (g.marks || 0) >= totalMarks * 0.9).length}
                </div>
                <p className="text-sm text-muted-foreground">Students scored 90%+</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
              ...
              <CardContent>
                <div className="text-3xl font-bold text-blue-600">
                  {grades.filter(g => (g.marks || 0) >= totalMarks * 0.7 && (g.marks || 0) < totalMarks * 0.9).length}
                </div>
                <p className="text-sm text-muted-foreground">Students scored 70-89%</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
              ...
              <CardContent>
                <div className="text-3xl font-bold text-red-600">
                  {grades.filter(g => (g.marks || 0) < totalMarks * 0.6).length}
                </div>
                <p className="text-sm text-muted-foreground">Students scored below 60%</p>
              </CardContent>
            </Card>
          </div>
        </div>
            ...
            {rankedResults.length > 0 ? (
              <div className="space-y-4">
                {rankedResults.map((result, index) => (
                  <div key={result.grade.id} className="flex items-center justify-between p-4 border rounded-lg hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-medium">{result.student?.user.name}</h3>
                        <p className="text-sm text-muted-foreground">{result.student?.user.email}</p>
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-bold">{result.grade.marks}/{exam.totalMarks}</p>
                      <p className="text-lg font-medium text-primary">{result.percentage}%</p>
                      <p className={`text-sm font-bold ${
                        result.grade.grade === 'A' ? 'text-green-600' :
                        result.grade.grade === 'B' ? 'text-blue-600' :
                        result.grade.grade === 'C' ? 'text-yellow-600' : 'text-red-600'
                      }`}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">{examStats.median}</div>
                  <p className="text-sm text-muted-foreground">Median Score</p>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600">{standardDeviation}</div>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="p-4 bg-blue-50 rounded-lg">
                  <h4 className="font-medium text-blue-800">Class Performance</h4>
                ...
                <div className="p-4 bg-green-50 rounded-lg">
                  <h4 className="font-medium text-green-800">Score Distribution</h4>
                ...
                <div className="p-4 bg-yellow-50 rounded-lg">
                  ...
                  <ul className="text-sm text-yellow-700 list-disc list-inside space-y-1">
                    <li>Provide additional support for students who scored below 60%</li>
                    <li>Consider enrichment activities for top performers</li>
                    <li>Review questions where most students struggled</li>
                    <li>Adjust teaching methods based on performance patterns</li>
                  </ul>
                </div>
              </div>
            </CardContent>
