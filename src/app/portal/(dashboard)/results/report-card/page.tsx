  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <Select value={selectedStudent} onValueChange={setSelectedStudent}>
                ...
                <SelectContent>
                  {Array.isArray(students?.students) && students.students.map((student: any) => (
      ...
      {error && (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <Button onClick={() => selectedStudent && getReportCard(parseInt(selectedStudent), academicYear, term)}>
              Retry
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="print:bg-white print:text-black">
          ...
          <Card className="print:shadow-none print:border-2">
            <CardContent className="text-center py-8">
              ...
              <h2 className="text-xl font-semibold mt-4">STUDENT'S CONTINUOUS ASSESSMENT REPORT SHEET</h2>
            </CardContent>
          </Card>
          ...
          <Card className="print:shadow-none print:border-2">
            <CardContent className="py-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <span className="font-semibold">NAME:</span> {reportCard.student?.user?.firstname} {reportCard.student?.user?.lastname}
                </div>
                <div>
                  <span className="font-semibold">CLASS:</span> {reportCard.class?.name}
                </div>
                <div>
                  <span className="font-semibold">SESSION:</span> {reportCard.academicYear}
                </div>
                <div>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-gray-50 print:bg-gray-100">
                      ...
                      <th className="border p-2 text-center">Overall (c+f)/2</th>
                  ...
                  <tbody>
                    {reportCard.results.map((result, index) => (
                      <tr key={index} className="hover:bg-gray-50">
                        <td className="border p-2 font-medium">{result.subject?.name}</td>
                        <td className="border p-2 text-center">{result.ca1?.toFixed(1) || "-"}</td>
                        <td className="border p-2 text-center">{result.ca2?.toFixed(1) || "-"}</td>
                        <td className="border p-2 text-center">{result.caTotal?.toFixed(1) || "-"}</td>
                        <td className="border p-2 text-center">{result.examScore?.toFixed(1) || "-"}</td>
                        <td className="border p-2 text-center">{result.totalScore?.toFixed(1) || "-"}</td>
                        <td className="border p-2 text-center">{result.ltc?.toFixed(1) || "-"}</td>
                        <td className="border p-2 text-center font-semibold">{result.overallScore?.toFixed(1) || "-"}</td>
                        <td className="border p-2 text-center">{result.classAverage?.toFixed(1) || "-"}</td>
                        ...
                        <td className="border p-2 text-center">
                          {result.subjectPosition ? `${result.subjectPosition}${getOrdinalSuffix(result.subjectPosition)}` : "-"}
                        </td>
                        <td className="border p-2 text-center">{result.remark || getGradeRemark(result.grade || "")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <Card className="print:shadow-none print:border-2">
            <CardContent className="py-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <span className="font-semibold">Number of Subjects:</span> {reportCard.summary.numberOfSubjects}
                </div>
                <div>
                  <span className="font-semibold">Marks Obtainable:</span> {reportCard.summary.marksObtainable}
                </div>
                <div>
                  <span className="font-semibold">Total Marks Obtained:</span> {reportCard.summary.totalMarksObtained.toFixed(0)}
                </div>
                <div>
                  <span className="font-semibold">Average:</span> {reportCard.summary.average.toFixed(1)}
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      {!reportCard && !loading && !error && selectedStudent && (
        <Card>
          <CardContent className="text-center py-12">
            <p className="text-muted-foreground">No results found for this student and term.</p>
          </CardContent>
        </Card>
      )}
    </div>
